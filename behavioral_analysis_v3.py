...
file_path = "%sPurchaseAll.metis"%(src_path)
with open(file_path, 'w') as f:
    f.write("%s %s\n" % (len(Node), len(transaction_sorted)))
    for node in Node:
        node_neighbors = []
        for i in range(len(transaction_sorted)):
            if transaction_sorted[i]['seller'] == node \
            and transaction_sorted[i]['buyer'] not in node_neighbors:
                node_neighbors.append(transaction_sorted[i]['buyer'])
                curNum = Node.index(transaction_sorted[i]['buyer']) + 1
                f.write("%d " % curNum)
            elif transaction_sorted[i]['buyer'] == node \
            and transaction_sorted[i]['seller'] not in node_neighbors:
                node_neighbors.append(transaction_sorted[i]['seller'])
                curNum = Node.index(transaction_sorted[i]['seller']) + 1
                f.write("%d " % curNum)
        if not node == Node[len(Node) - 1]:
            f.write("\n")
...
file_path = "%sPurchaseAll.metis.c1000.i2.0.b0.5"%(src_path)
with open(file_path, 'r') as f:
    content = f.readlines()
content = [x.strip() for x in content]
Community = {}
for i in range(len(content)):
    Community[Node[i]] = content[i]

# list of clustered dictionary
for t in range(len(ClusterAll)): # now only consider from 01 to 09
    tempDict = {}
    for index in ClusterNodeT[t]:
        # first discovery the communities for each snapshot
        tempDict[index] = Community[index]
    dictList.append(cluster(tempDict))
#print (dictList[0]) # use cluster function
...
for t in range(T_start, T_end): #range(1, len(ClusterAll)-1): # snapshots 0-23
    ...
    for key in dictList[t]:
        ...
        if key in dictList[t-1]:
            joinIndex = True
            joinNumber += len(set(dictList[t][key]) - set(dictList[t-1][key]))
            joinCount += len(set(dictList[t][key]) - set(dictList[t-1][key]))
    ...
    for key in dictList[t]:
        ...
        if key in dictList[t+1]:
            leaveIndex = True
            leaveNumber += len(set(dictList[t][key]) - set(dictList[t+1][key]))
            leaveCount += len(set(dictList[t][key]) - set(dictList[t+1][key]))
...
for t in range(T_start, T_end):
    tempDict = {}
    for key in joinListCount[t]:
        if key in leaveListCount[t]:
            SI = len(dictList[t][key]) / (joinListCount[t][key] + leaveListCount[t][key])
            tempDict[key] = SI
        else:
            SI = len(dictList[t][key]) / (joinListCount[t][key])
            tempDict[key] = SI
    
    for key in leaveListCount[t]:
        if key not in tempDict:
            SI = len(dictList[t][key]) / (leaveListCount[t][key])
            tempDict[key] = SI
    SIList.append(tempDict)
#print (SIList)  
...
keyList = []
for node in Node:
    if not Community[node] in keyList:
        keyList.append(Community[node])

for key in keyList:
    temp = 0
    for t in range(len(ClusterAll)):
        if key in dictList[t]:
            temp += 1
    activity[key] = temp
#print (activity)
for t in range(T_start, T_end):
    ...
    for key in joinListCount[t]:
        if key in leaveListCount[t]:
            SoI = (joinListCount[t][key] + leaveListCount[t][key]) / activity[key]
            tempDict[key] = SoI
        else:
            SoI = (joinListCount[t][key]) / activity[key]
            tempDict[key] = SoI
    
    for key in leaveListCount[t]:
        if key not in tempDict:
            SoI = (leaveListCount[t][key]) / activity[key]
            tempDict[key] = SoI
    SoIList.append(tempDict)
#print (SoIList)       
...
for t in range(T_start, T_end):
    ...
    for key in joinListCount[t]:
        if key in leaveListCount[t]:
            PI = (joinListCount[t][key] - leaveListCount[t][key])
            tempDict[key] = PI
        else:
            PI = joinListCount[t][key]
            tempDict[key] = PI
    
    for key in leaveListCount[t]:
        if key not in tempDict:
            PI = -leaveListCount[t][key]
            tempDict[key] = PI
    PIList.append(tempDict)
#print (PIList)  
...
for node in Node:
    ...
    for t in range(T_start, T_end):
        if node in ClusterNodeT[t]:
            Move += 1
            if Community[node] in leaveListCount[t]:
                if Community[node] in joinListCount[t]:
                    Companion += (joinListCount[t][Community[node]] + leaveListCount[t][Community[node]])
                else:
                    Companion += leaveListCount[t][Community[node]]
            else:
                if Community[node] in joinListCount[t]:
                    Companion += (joinListCount[t][Community[node]])
            
    tempDict['value'] = Companion / Move
    InfList.append(tempDict)

InfSorted = sorted(InfList, key = lambda i: i['value'])
